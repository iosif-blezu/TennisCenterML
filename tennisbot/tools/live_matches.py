...
print(f"[LiveMatchesTool] Cache backend: {cache.__class__.__name__}")
print(f"[LiveMatchesTool] Cache name: {cache.cache_name}.sqlite")
print(f"[LiveMatchesTool] TTL (seconds): {cfg.CACHE_TTL['live']}")
...
class LiveMatchesTool(BaseTool):
    ...
    def _run(
        ...
        try:
            resp = requests.get(cfg.endpoint_live, timeout=10)
            data = resp.json()
            # Log cache usage
            from_cache = getattr(resp, "from_cache", False)
            print(f"[LiveMatchesTool] Request URL: {resp.url}")
            print(f"[LiveMatchesTool] Response from cache: {from_cache}")
